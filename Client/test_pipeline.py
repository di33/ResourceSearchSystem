...
所有默认配置从 client/.env 和 client/.env.local 文件读取，命令行参数可覆盖。
...
修改 .env 中的 CLIENT_LLM_PROVIDER，并把 API Key 放到 .env.local，即可切换 provider，
无需改命令行。
"""
...
_REPO_ROOT = Path(_SCRIPT_DIR).resolve().parent
_CLIENT_SCRIPTS = os.path.join(_SCRIPT_DIR, "Scripts")
_SERVER_SCRIPTS = os.path.join(_REPO_ROOT, "server", "Scripts")
...
def _load_dotenv_files(*paths: str) -> dict[str, str]:
    env: dict[str, str] = {}
    for path in paths:
...
def _init_env():
    """Load client .env files and set missing os.environ entries so providers pick up API keys."""
...
def _env(key: str, fallback: str = "") -> str:
    """Read a config value: os.environ > client .env files > fallback."""
...
def main() -> int:
    ...
    work_dir = (
        ...
        if args.work_dir
        else str(_REPO_ROOT / "data" / "workdirs" / "test_workdir")
    )
    config_path = os.path.abspath(args.config) if args.config else os.path.join(_SCRIPT_DIR, "resource_types.json")
