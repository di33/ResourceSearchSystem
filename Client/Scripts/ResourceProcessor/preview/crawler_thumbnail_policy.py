...
def _pack_item_source_path(item: str | dict) -> str:
    if not isinstance(item, dict):
        return str(item)
    if str(item.get("resource_type") or "") == "single_image":
...
def _open_pack_collage_image(item: str | dict, *, prefer_source: bool) -> Image.Image:
    ...
    for path in (primary, fallback):
        if not path or not Path(path).is_file():
            continue
        try:
            if prefer_source and Path(path).suffix.lower() in RASTER_EXTS:
...
def _best_ordered_grid_cols(
    ...
    max_cols = max(1, min(max_cols, item_count))
    item_aspect = max(0.01, item_aspect)
    canvas_aspect = max(0.01, canvas_aspect)

    default_cols = min(max_cols, max(1, math.ceil(math.sqrt(item_count))))
    best: tuple[tuple[float, float, int, int], int] | None = None
    for cols in range(1, max_cols + 1):
        rows = max(1, math.ceil(item_count / cols))
        cell_w = canvas_aspect / cols
        cell_h = 1.0 / rows
        scale = min(cell_w / item_aspect, cell_h)
        display_w = item_aspect * scale
        display_h = scale
        display_area = item_count * display_w * display_h
        empty_cells = cols * rows - item_count
        score = (
            display_area,
            scale,
            -empty_cells,
            -abs(cols - default_cols),
        )
        if best is None or score > best[0]:
            best = (score, cols)
    return best[1] if best else default_cols


def _pack_collage_grid_cols(
    images: list[Image.Image],
    item_count: int,
    forced_grid_cols: int | None = None,
) -> int | None:
    item_count = max(1, item_count)
    if item_count <= 1:
        return 1
    ...
    if forced_grid_cols:
        max_cols = min(forced_grid_cols, item_count)
    else:
        max_cols = 6 if item_count > 9 else 3
    return _best_ordered_grid_cols(item_count, width / max(1, height), max_cols)
...
def _paste_pack_grid_collage(
    ...
    if grid_cols:
        cols = min(grid_cols, max(1, item_count))
    else:
        max_cols = 6 if item_count > 9 else 3
        cols = min(max_cols, max(1, math.ceil(math.sqrt(max(item_count, 1)))))
    rows = max(1, math.ceil(max(item_count, 1) / cols))
    cell_w = collage_size // cols
    cell_h = collage_size // rows
    padding = 4 if min(cell_w, cell_h) >= 48 else 2
    for idx, item in enumerate(items[:item_count]):
        row = idx // cols
        col = idx % cols
        x0 = collage_left + col * cell_w
        y0 = collage_top + row * cell_h
        with _open_pack_collage_image(item, prefer_source=False) as img:
            img.thumbnail((cell_w - padding * 2, cell_h - padding * 2))
            paste_x = x0 + (cell_w - img.width) // 2
            paste_y = y0 + (cell_h - img.height) // 2
            _paste_image(sheet, img, (paste_x, paste_y))
...
def _best_pack_atlas_layout(
    ...
    for _ in range(24):
        mid = (low + high) / 2
        candidate = _try_pack_atlas_layout(sizes, max_width, max_height, gap, mid)
        if candidate:
            best = candidate
            low = mid
        else:
    ...
    fallback = _try_pack_atlas_layout(sizes, max_width, max_height, gap, max(scale, 0.01))
    if fallback:
        return fallback
    return {0: (0, 0, max_width, max(1, int(max_source_h * max_width / max_source_w)))}, max_width, max_height
...
def _render_dense_atlas_image(
    ...
    *,
    ...
    gap = 8 if max_width >= 768 else 6 if max_width >= 384 else 4
    sizes = [(max(1, image.width), max(1, image.height)) for image in prepared]
    positions, used_width, used_height = _best_pack_atlas_layout(sizes, max_width, max_height, gap)
    ...
    for idx, image in enumerate(prepared):
        x, y, width, height = positions.get(idx, (0, 0, 1, 1))
        tile = image
...
def _save_pack_collage(
    ...
    if items:
        ...
        ordered_grid_cols = _pack_collage_grid_cols(layout_images, item_count, grid_cols)
        if ordered_grid_cols:
            _paste_pack_grid_collage(sheet, items, collage_left, collage_top, collage_size, max_items, ordered_grid_cols)
        else:
            layout_size = PACK_ATLAS_LAYOUT_SIZE if len(layout_images) > 1 else collage_size
            _paste_pack_atlas_collage(sheet, layout_images, collage_left, collage_top, collage_size, layout_size)
    if not image_paths:
...
PACK_AUDIO_RESOURCE_TYPE = "audio_file"
...
PACK_SINGLE_IMAGE_GALLERY_PAGE_SIZE = 256
PACK_SINGLE_IMAGE_GRID_MAX_COLS = 16
PACK_DEFAULT_GALLERY_PAGE_SIZE = 9
PACK_AUDIO_SAMPLE_GROUP_LIMIT = 8
DENSE_ATLAS_LAYOUT_SIZE = 1024
PACK_ATLAS_LAYOUT_SIZE = DENSE_ATLAS_LAYOUT_SIZE
PACK_DYNAMIC_PAGE_CANVAS_SIZE = PACK_GENERATED_COLLAGE_SIZE - 64
PACK_DYNAMIC_PAGE_FILL_TARGET = 0.72
PACK_DYNAMIC_PAGE_MIN_FILL = 0.35
PACK_DYNAMIC_PAGE_MIN_SCALE_RATIO = 0.55
PACK_DYNAMIC_PAGE_ABSOLUTE_MIN_SCALE = 0.08
PACK_DYNAMIC_PAGE_MIN_ITEM_EDGE = 24
PACK_DYNAMIC_PAGE_READABLE_EDGE = 32
PACK_DYNAMIC_PAGE_MIN_ITEMS_BEFORE_FILL_STOP = 12
PACK_DYNAMIC_PAGE_MIN_FILL_GAIN = 0.005
PACK_DYNAMIC_PAGE_MAX_ITEMS = PACK_SINGLE_IMAGE_GALLERY_PAGE_SIZE
PACK_DYNAMIC_PAGE_SCAN_LIMIT = PACK_SINGLE_IMAGE_GALLERY_PAGE_SIZE
...
def _filter_pack_alternate_format_records(
    ...
    for _, _, _, record in candidates:
        if str(record.get("resource_type") or "") != "single_image":
            continue
...
def _pack_child_preview_records(entity: ResourceProcessingEntity) -> list[dict]:
    ...
    has_visual_sheet = any(
        ...
        and (
            str(item.get("resource_type") or "") in {"atlas", "tileset", "tiled_map"}
...
def _pack_record_is_map_like(record: dict) -> bool:
    if str(record.get("resource_type") or "") == "tiled_map":
        return True
    values = [
...
def _pack_allows_generated_showcase(entity: ResourceProcessingEntity, records: list[dict]) -> bool:
    if "tiled_map" in set(entity.contains_resource_types or []):
        return True
...
def _pack_preview_page_size(records: list[dict], entity: ResourceProcessingEntity | None = None) -> int:
    if len(records) > PACK_DEFAULT_GALLERY_PAGE_SIZE and _pack_is_single_image_collection(records):
        return PACK_SINGLE_IMAGE_GALLERY_PAGE_SIZE
    return PACK_DEFAULT_GALLERY_PAGE_SIZE
...
def _pack_preview_grid_cols(records: list[dict], entity: ResourceProcessingEntity) -> int | None:
    if _pack_is_single_image_collection(records):
        return PACK_SINGLE_IMAGE_GRID_MAX_COLS
    return None


def _fixed_pack_preview_pages(records: list[dict], entity: ResourceProcessingEntity | None = None) -> list[list[dict]]:
    page_size = _pack_preview_page_size(records, entity)
...
def _pack_collage_item_sizes(items: list[str | dict]) -> list[tuple[int, int]]:
    sizes: list[tuple[int, int]] = []
    for item in items:
        image = _open_pack_collage_image(item, prefer_source=True)
        sizes.append((max(1, image.width), max(1, image.height)))
        image.close()
    return sizes


def _pack_page_layout_metrics_for_sizes(sizes: list[tuple[int, int]]) -> dict[str, float]:
    if not sizes:
        return {"fill": 0.0, "scale": 1.0, "min_edge": 0.0}
    layout_size = max(PACK_DYNAMIC_PAGE_CANVAS_SIZE, PACK_ATLAS_LAYOUT_SIZE)
    preview_ratio = PACK_DYNAMIC_PAGE_CANVAS_SIZE / float(layout_size)
    gap = 8 if layout_size >= 768 else 6 if layout_size >= 384 else 4
    positions, _, _ = _best_pack_atlas_layout(
        sizes,
        layout_size,
        layout_size,
        gap,
    ...
    area = 0
    scales: list[float] = []
    min_edge = float(PACK_DYNAMIC_PAGE_CANVAS_SIZE)
    for idx, (source_w, source_h) in enumerate(sizes):
        _, _, width, height = positions.get(idx, (0, 0, 1, 1))
        area += width * height
        scales.append(min(width / source_w, height / source_h) * preview_ratio)
        min_edge = min(min_edge, width * preview_ratio, height * preview_ratio)
    return {
        "fill": area / float(layout_size * layout_size),
        "scale": min(scales) if scales else 1.0,
        "min_edge": min_edge,
    }


def _pack_page_layout_metrics(items: list[str | dict]) -> dict[str, float]:
    return _pack_page_layout_metrics_for_sizes(_pack_collage_item_sizes(items))


def _pack_should_start_new_dynamic_page(
    current: list[str | dict],
    current_metrics: dict[str, float],
    candidate_metrics: dict[str, float],
) -> bool:
    if len(current) >= PACK_DYNAMIC_PAGE_MAX_ITEMS:
        return True
    current_fill = current_metrics["fill"]
    candidate_fill = candidate_metrics["fill"]
    current_scale = current_metrics["scale"]
    candidate_scale = candidate_metrics["scale"]
    if current_fill >= PACK_DYNAMIC_PAGE_MIN_FILL:
        if candidate_scale < current_scale * PACK_DYNAMIC_PAGE_MIN_SCALE_RATIO:
            return True
        if (
            candidate_metrics["min_edge"] < PACK_DYNAMIC_PAGE_MIN_ITEM_EDGE
            and current_metrics["min_edge"] >= PACK_DYNAMIC_PAGE_MIN_ITEM_EDGE
        ):
            return True
    if current_fill >= PACK_DYNAMIC_PAGE_FILL_TARGET:
        if (
            len(current) < PACK_DYNAMIC_PAGE_MIN_ITEMS_BEFORE_FILL_STOP
            and candidate_metrics["min_edge"] >= PACK_DYNAMIC_PAGE_READABLE_EDGE
        ):
            return False
        fill_gain = candidate_fill - current_fill
        if fill_gain < PACK_DYNAMIC_PAGE_MIN_FILL_GAIN:
            return True
    return False


def _pack_dynamic_page_candidate_allowed(
    current_metrics: dict[str, float],
    candidate_metrics: dict[str, float],
) -> bool:
    if candidate_metrics["scale"] < PACK_DYNAMIC_PAGE_ABSOLUTE_MIN_SCALE:
        return False
    if current_metrics["fill"] >= PACK_DYNAMIC_PAGE_MIN_FILL:
        if candidate_metrics["scale"] < current_metrics["scale"] * PACK_DYNAMIC_PAGE_MIN_SCALE_RATIO:
            return False
        if (
            candidate_metrics["min_edge"] < PACK_DYNAMIC_PAGE_MIN_ITEM_EDGE
            and current_metrics["min_edge"] >= PACK_DYNAMIC_PAGE_MIN_ITEM_EDGE
            ...
            return False
    if current_metrics["fill"] < PACK_DYNAMIC_PAGE_MIN_FILL or candidate_metrics["scale"] >= current_metrics["scale"] * 0.995:
        min_gain = 0.0
    else:
        min_gain = PACK_DYNAMIC_PAGE_MIN_FILL_GAIN
    return candidate_metrics["fill"] > current_metrics["fill"] + min_gain


def _select_pack_dynamic_page_candidate(
    current_sizes: list[tuple[int, int]],
    remaining: list[tuple[str | dict, tuple[int, int]]],
    current_metrics: dict[str, float],
) -> tuple[int, dict[str, float]] | None:
    best: tuple[tuple[float, float, float], int, dict[str, float]] | None = None
    for index, (_, size) in enumerate(remaining[:PACK_DYNAMIC_PAGE_SCAN_LIMIT]):
        if len(current_sizes) >= PACK_DYNAMIC_PAGE_MAX_ITEMS:
            break
        candidate_metrics = _pack_page_layout_metrics_for_sizes([*current_sizes, size])
        if not _pack_dynamic_page_candidate_allowed(current_metrics, candidate_metrics):
            continue
        fill_gain = candidate_metrics["fill"] - current_metrics["fill"]
        scale_drop = max(0.0, current_metrics["scale"] - candidate_metrics["scale"])
        min_edge = candidate_metrics["min_edge"]
        score = (fill_gain - scale_drop * 0.35, candidate_metrics["fill"], min_edge)
        if best is None or score > best[0]:
            best = (score, index, candidate_metrics)
    if best is None:
        return None
...
def _pack_dynamic_preview_pages(records: list[str | dict]) -> list[list[str | dict]]:
    ...
    sized_records = list(zip(records, _pack_collage_item_sizes(list(records))))
    for record, size in sized_records:
        if not current:
            current = [record]
            current_sizes = [size]
            current_metrics = _pack_page_layout_metrics_for_sizes(current_sizes)
            continue

        candidate_sizes = [*current_sizes, size]
        candidate_metrics = _pack_page_layout_metrics_for_sizes(candidate_sizes)
        if current_metrics and _pack_should_start_new_dynamic_page(current, current_metrics, candidate_metrics):
...
def _pack_dynamic_preview_pages_sequential(records: list[str | dict]) -> list[list[str | dict]]:
    pages: list[list[str | dict]] = []
    current: list[str | dict] = []
    current_metrics: dict[str, float] | None = None
    for record in records:
        if not current:
            current = [record]
            current_metrics = _pack_page_layout_metrics(current)
            continue
        candidate = [*current, record]
        candidate_metrics = _pack_page_layout_metrics(candidate)
        if current_metrics and _pack_should_start_new_dynamic_page(current, current_metrics, candidate_metrics):
            pages.append(current)
            current = [record]
            current_metrics = _pack_page_layout_metrics(current)
...
def _pack_preview_pages(records: list[dict], entity: ResourceProcessingEntity | None = None) -> list[list[dict]]:
    if not records:
        return []
    if _pack_is_single_image_collection(records):
        return _fixed_pack_preview_pages(records, entity)
    sizes = _pack_collage_item_sizes(records[: min(len(records), PACK_DYNAMIC_PAGE_MAX_ITEMS)])
    if sizes and len({size for size in sizes}) == 1:
        return _fixed_pack_preview_pages(records, entity)
    return _pack_dynamic_preview_pages(records)


def _pack_direct_image_pages(image_paths: list[str]) -> list[list[str]]:
    paths = sorted(image_paths, key=_natural_sort_key)
    if not paths:
        return []
    return [paths[index : index + PACK_SINGLE_IMAGE_GALLERY_PAGE_SIZE] for index in range(0, len(paths), PACK_SINGLE_IMAGE_GALLERY_PAGE_SIZE)]
...
class CrawlerThumbnailPolicy:
    ...
    async def generate_previews(self, entity: ResourceProcessingEntity) -> list[PreviewInfo]:
        ...
        if resource_type == "spine_skeleton":
    ...
    async def _generate_tileset_previews(self, entity: ResourceProcessingEntity) -> list[PreviewInfo]:
        ...
        output_dir = self._output_dir_for(entity)
        overview_path = _find_tileset_overview_image(image_paths)
        ...
        if overview_path:
            ...
            await asyncio.get_running_loop().run_in_executor(None, _save_tileset_sheet, image_paths, primary_path, self.max_size)
    ...
    async def _generate_pack_previews(self, entity: ResourceProcessingEntity) -> list[PreviewInfo]:
        ...
        if child_preview_pages:
            ...
            grid_cols = _pack_preview_grid_cols(child_preview_records, entity)
            ...
            if len(first_page) == 1 and len(child_preview_pages) == 1:
                preview_path = _pack_item_preview_path(first_page[0])
                await asyncio.get_running_loop().run_in_executor(
                    None,
                    _save_existing_raster_preview,
                    preview_path,
        ...
        if image_paths:
            ...
            await asyncio.get_running_loop().run_in_executor(
                ...
                direct_collage_size,
                len(image_pages[0]),
                PACK_SINGLE_IMAGE_GRID_MAX_COLS,
            )
