...
def main() -> int:
    parser = make_arg_parser(
        ...
        extra_args=[
            ...
            ("--no-wait", {"action": "store_true", "dest": "detach", "default": True, "help": "异步入队后立即退出（默认行为；参数为兼容旧命令保留）"}),
            ("--wait-all", {"action": "store_false", "dest": "detach", "help": "全部任务入队后持续批量对账，直到本地没有 queued/submitting 任务"}),
