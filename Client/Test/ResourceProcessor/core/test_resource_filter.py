...
from ResourceProcessor.core.resource_filter import filter_resources, copy_and_categorize_resources, detect_malicious_file, generate_resource_index, filter_resources_with_handlers, ResourceHandlerFactory, check_file_integrity, PreviewGenerator
...
class TestResourceFilter(unittest.TestCase):
    ...
    def test_generate_resource_index(self):
        ...
        from ResourceProcessor.core.resource_filter import generate_resource_index
...
class TestPreviewGenerator(unittest.TestCase):
    ...
    @patch("ResourceProcessor.core.resource_filter.subprocess.run", side_effect=_mock_ffmpeg_run)
...
class TestGroupFilesByDirectory(unittest.TestCase):
    """Tests for group_files_by_directory."""

    def test_single_directory(self):
        from ResourceProcessor.core.resource_filter import group_files_by_directory
        files = ["/res/model.fbx", "/res/texture.png", "/res/normal.png"]
    ...
    def test_multiple_directories(self):
        from ResourceProcessor.core.resource_filter import group_files_by_directory
        files = ["/dir_a/a.png", "/dir_a/b.png", "/dir_b/c.fbx"]
        groups = group_files_by_directory(files)
        assert len(groups) == 2
...
class TestDetermineFileRole(unittest.TestCase):
    """Tests for determine_file_role."""

    def test_model_is_primary(self):
        from ResourceProcessor.core.resource_filter import determine_file_role
        role, is_primary = determine_file_role("/res/chair.fbx", ["/res/chair.fbx"])
        assert role == "model"
        assert is_primary is True

    def test_gltf_is_primary(self):
        from ResourceProcessor.core.resource_filter import determine_file_role
        role, is_primary = determine_file_role("/res/scene.glb", ["/res/scene.glb"])
        assert role == "model"
        assert is_primary is True

    def test_texture_named_file(self):
        from ResourceProcessor.core.resource_filter import determine_file_role
        role, _ = determine_file_role("/res/diffuse_texture.png", ["/res/diffuse_texture.png"])
        assert role == "texture"

    def test_normal_named_file(self):
        from ResourceProcessor.core.resource_filter import determine_file_role
        role, _ = determine_file_role("/res/normal_map.jpg", ["/res/normal_map.jpg"])
        assert role == "texture"

    def test_plain_image_is_main(self):
        from ResourceProcessor.core.resource_filter import determine_file_role
        role, is_primary = determine_file_role("/res/photo.jpg", ["/res/photo.jpg"])
        assert role == "main"
        assert is_primary is False

    def test_unknown_extension(self):
        from ResourceProcessor.core.resource_filter import determine_file_role
        role, is_primary = determine_file_role("/res/readme.txt", ["/res/readme.txt"])
        assert role == "attachment"
        assert is_primary is False


class TestComputeCompositeMd5(unittest.TestCase):
    """Tests for compute_composite_md5."""

    def setUp(self):
        self.test_dir = os.path.join(os.environ.get("TEMP", "/tmp"), "test_composite")
        os.makedirs(self.test_dir, exist_ok=True)
        self.file_a = os.path.join(self.test_dir, "a.txt")
        self.file_b = os.path.join(self.test_dir, "b.txt")
    ...
    def test_deterministic(self):
        from ResourceProcessor.core.resource_filter import compute_composite_md5
        h1 = compute_composite_md5([self.file_a, self.file_b])
        h2 = compute_composite_md5([self.file_a, self.file_b])
        assert h1 == h2

    def test_order_independent(self):
        from ResourceProcessor.core.resource_filter import compute_composite_md5
        h1 = compute_composite_md5([self.file_a, self.file_b])
        h2 = compute_composite_md5([self.file_b, self.file_a])
        assert h1 == h2

    def test_different_content_different_hash(self):
        from ResourceProcessor.core.resource_filter import compute_composite_md5
        h1 = compute_composite_md5([self.file_a])
        h2 = compute_composite_md5([self.file_b])
        assert h1 != h2

    def test_returns_hex_string(self):
        from ResourceProcessor.core.resource_filter import compute_composite_md5
        h = compute_composite_md5([self.file_a])
        assert len(h) == 32
        int(h, 16)
