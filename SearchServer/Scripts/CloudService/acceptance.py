...
def build_default_checklist() -> AcceptanceChecklist:
    ...
    cl.add(AcceptanceItem(
        ...
        title="密钥本地存储",
        description="本地密钥不发送到云端，优先使用平台原生密钥管理",
    ...
    cl.add(AcceptanceItem(
        ...
        title="本地状态机完整闭环",
        description="discovered → preview_ready → description_ready → embedding_ready → package_ready → upserted → committed → synced",
        target="状态机全路径可达",
...
def build_delivery_plan() -> List[DeliveryStage]:
    ...
    return [
        ...
        DeliveryStage(
            stage=3, name="资源入库协议联调",
            objective="打通加工服务器到 SearchServer 的 upsert 入库流程",
