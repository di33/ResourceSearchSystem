"""Object URL profile configuration.

A storage profile describes how an object key maps to a public URL. Databases
and manifests should store the stable profile id plus object_key, while CDN
and auth details stay in environment or secret-backed configuration.
"""
...
def _resolve_env_refs(data: dict[str, Any], env: Mapping[str, str]) -> dict[str, Any]:
    """Resolve config values like {"cdn_auth_key_env": "CDN_AUTH_KEY"}."""
    resolved = dict(data)
...
def _profile_from_dict(profile_id: str, raw: Mapping[str, Any], env: Mapping[str, str]) -> StorageProfile:
    ...
    return StorageProfile(
        ...
        url_mode=str(data.get("url_mode") or "cdn_unsigned"),
...
def _default_profiles_files() -> tuple[Path, ...]:
    ...
    return (
        profile_root / "storage_profiles.jsonc",
        profile_root / "storage_profiles.json",
    )
...
def load_storage_profiles(env: Mapping[str, str] | None = None) -> StorageProfileRegistry:
    ...
    if not profiles:
        raise ValueError("no storage profiles configured; configure SearchServer/storage_profiles.jsonc")
