...
class _FakeStorage:
    ...
    def generate_download_url(self, key, expires=None, storage_profile_id=""):
        self.url_calls.append((key, expires, storage_profile_id))
...
class TestMilvusSearchClient(unittest.IsolatedAsyncioTestCase):
    ...
    async def _seed_search_task(self, session):
        task = ResourceTask(
            ...
            source_storage_profile_id="default",
            source_object_key="downloads/res-search-001/stone-floor.zip",
            source_object_file_name="stone-floor.zip",
            source_object_file_format="zip",
            source_object_file_size=4096,
            package_storage_profile_id="package-profile",
            package_object_key="packages/source-pack.zip",
        ...
        session.add(
            ResourceFile(
                ...
                object_key="files/res-search-001/floor.png",
                is_primary=True,
            )
        )
        session.add(
    ...
    async def test_search_builds_results_from_milvus_and_database(self):
        ...
        self.assertNotIn("entries", item.to_dict()["file_structure"])
        ...
        session.add_all(
            [
                ...
                ResourceFile(
                    ...
                    file_format="fbx",
                    content_md5="fbx-md5",
                    object_key="files/res-pack-001/model.fbx",
                    is_primary=True,
                ),
                ResourceFile(
                    task_id=task.id,
                    ...
                    file_format="png",
                    content_md5="png-md5",
                    object_key="files/res-pack-001/preview.png",
                    is_primary=False,
                ),
            ]
        )
    ...
    async def test_search_result_omits_legacy_parent_fields_and_returns_package_object(self):
        session = self.session_factory()
        task = ResourceTask(
            content_md5="md5-child",
            resource_type="single_image",
            source_directory="assets/asset",
            process_state="committed",
        ...
        session.add_all(
            [
                ...
                ResourceFile(
                    task_id=task.id,
                    file_path="assets/asset/image.png",
                    file_name="image.png",
                    file_size=256,
                    file_format="png",
                    content_md5="child-file-md5",
                    object_key="files/res-child-001/image.png",
                    is_primary=True,
                ),
            ]
        )
        ...
        item = response.results[0]
        self.assertEqual(
            item.package_download_url,
            "https://storage.local/packages/source-pack.zip",
        )
        result_dict = item.to_dict()
        self.assertNotIn("parent_resource_id", result_dict)
        self.assertNotIn("parent_title", result_dict)
        self.assertNotIn("parent_preview_url", result_dict)
        self.assertNotIn("parent_download_url", result_dict)
        self.assertNotIn("child_resource_count", result_dict)
        self.assertNotIn("contains_resource_types", result_dict)
    ...
    async def test_get_download_link_prefers_package_object_and_expiry(self):
        session = self.session_factory()
        await self._seed_search_task(session)
        fake_storage = _FakeStorage()
        ...
        self.assertEqual(response.file_name, "source-pack.zip")
        self.assertEqual(response.file_size, 0)
        self.assertEqual(response.content_type, "application/octet-stream")
        self.assertEqual(
            response.download_url,
            "https://storage.local/packages/source-pack.zip?expires=600",
        )
        ...
        self.assertEqual(fake_storage.url_calls[-1], ("packages/source-pack.zip", 600, "package-profile"))
