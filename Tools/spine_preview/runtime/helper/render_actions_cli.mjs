...
async function renderFrame({ page, baseUrl, outPath, width, height, animationName, time, frameCount, timeoutMs }) {
  const url = `${baseUrl}/renderer.html?width=${width}&height=${height}&frames=${frameCount}&animation=${encodeURIComponent(animationName)}&time=${encodeURIComponent(String(time))}&timeout=${timeoutMs}`;
  await page.goto(url, { waitUntil: "load", timeout: timeoutMs });
  await page.waitForFunction(
    () => document.body.getAttribute("data-ready") === "1" || document.body.getAttribute("data-error") === "1",
    null,
    { timeout: timeoutMs },
  );
  const pageError = await page.evaluate(() => window.__spinePreviewError || "");
  ...
  try {
    for (let index = 0; index < animations.length; index++) {
      ...
      for (let frame = 0; frame < frameCount; frame++) {
        samples.push(frameCount === 1 ? animation.duration / 2 : (animation.duration * frame) / (frameCount - 1));
      }
