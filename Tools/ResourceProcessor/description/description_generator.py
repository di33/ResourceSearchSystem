...
from resource_contracts.resource_types import AUDIO_FILE_RESOURCE_TYPE, TILESET_RESOURCE_TYPES
...
_TILESET_TYPES = set(TILESET_RESOURCE_TYPES)
...
class DescriptionInput:
    ...
    prompt_context_override: str = ""
    description_prompt_env: str = "LLM_DESCRIPTION_PROMPT"
    prompt_version_tag: str = ""
    attach_llm_media: bool = True
    ...
    def to_prompt_context(self) -> str:
        ...
        if self.prompt_context_override:
            return self.prompt_context_override
...
def resolve_prompt_version(base_version: str, input_data: DescriptionInput) -> str:
    tag = (input_data.prompt_version_tag or "").strip()
    if not tag:
        return base_version
    if tag in base_version.split("+"):
        return base_version
    return f"{base_version}+{tag}"
...
class MockLLMProvider(BaseMultiModalLLMProvider):
    ...
    async def generate_description(
        ...
        return build_description_result(
            ...
            main_content=main,
            detail_content=detail,
            prompt_version=resolve_prompt_version(self.PROMPT_VERSION, input_data),
        )
    ...
    async def classify_usage(
        ...
        if input_data.resolved_llm_input_type == "audio" or input_data.resource_type == AUDIO_FILE_RESOURCE_TYPE:
            return UsageClassification(
                space="通用",
                category="音频",
                subcategories=["音效"],
