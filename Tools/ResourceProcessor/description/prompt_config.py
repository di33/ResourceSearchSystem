...
DEFAULT_PACK_DESCRIPTION_PROMPT = (
    "你是游戏素材包描述生成助手。请根据已经用 embedding 聚类去重的子资源描述，"
    "总结整个资源包的主题、内容范围、风格特征和适用场景。不要逐条罗列子资源，"
    "不要把代表项简单相加，要提炼高频特征、共同用途和少量重要差异。"
)
...
def get_description_prompt(env_var: str = "LLM_DESCRIPTION_PROMPT") -> str:
    default_prompt = (
        DEFAULT_PACK_DESCRIPTION_PROMPT
        if env_var == "LLM_PACK_DESCRIPTION_PROMPT"
        else DEFAULT_DESCRIPTION_PROMPT
    )
    return _unescape(os.environ.get(env_var, "")) or default_prompt


def get_description_user_prompt(
    context: str,
    *,
    description_prompt_env: str = "LLM_DESCRIPTION_PROMPT",
) -> str:
    description_prompt = get_description_prompt(description_prompt_env)
    return (
